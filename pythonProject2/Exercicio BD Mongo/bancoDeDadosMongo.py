...
class BancoDeDados:
    ...
    def cadastro_novos_pets(self):
        ...
        pet = dict(nome=f"{nome}", animal=f"{animal}", raca=f"{raca}", idade=f"{idade}")
    ...
    def consulta_pet_por_nome(self):
        ...
        print(pd.DataFrame(self.informacoes.find({"nome": f"{nome}"})))

    def consulta_pet_por_raca(self):
        raca = input("Digite a raça que deseja consultar: ").capitalize()
        print(pd.DataFrame(self.informacoes.find({"raca": f"{raca}"})))

    def consulta_por_animal(self):
        animal = input("Digite o animal que deseja consultar: ").capitalize()
        print(pd.DataFrame(self.informacoes.find({"animal": f"{animal}"})))
    ...
    def deletar_cadastro(self):
        ...
        self.informacoes.delete_one({"_id": ObjectId(f"{id}")})
